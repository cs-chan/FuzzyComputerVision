# -*- coding: utf-8 -*-
"""
Created on Sun May 21 15:52:48 2017
...
#Edge detection
def FE_edgeDetection(image, method, display):
    if method == 'canny':
        edgesImage = cv2.Canny(image,100,200)
#    elif method == 'sobel':
#        edgesImage = cv2.Sobel(image,cv2.CV_16S,1,0)   
#    else:
#        print 'Please input a valid method: "canny","sobel", or..'
#        edgesImage = 0;
#        break
    if display:
        show(edgesImage) 
...
#Feature clustering
def FE_Clustering(feaMat, method, clusterNo):    
    if method == 'Crisp':
        ...
        cntr, u, u0, d, jm, p, fpc = fuzz.cluster.cmeans(feaMat.transpose(), clusterNo, 2, error=0.005, maxiter=1000)
        cluster_membership = np.argmax(u, axis=0)
        cluster_labels = cluster_membership
        cluster_centers = cntr
        cluster = copy.copy(cluster_centers)
        cluster_labels_unique = np.unique(cluster_labels) 
...
def FE_Quantisation(feaMat, method, cluster):
    if method == 'Crisp':
        cluster_membership = cluster.predict(feaMat)
        x = np.bincount(cluster_membership, minlength=len(cluster.cluster_centers_))
    elif method == 'Fuzzy':
        u, u0, d, jm, p, fpc = fuzz.cluster.cmeans_predict(feaMat.T, cluster, 2, error=0.005, maxiter=1000)
        cluster_membership = np.argmax(u, axis=0)
        x = np.bincount(cluster_membership, minlength=len(cluster))
    return x
...
def Image_Classification(trainingFolder, testingFolder, feature, classification_method):
    ...
    """
    #File setting
#    trainingFolder = "Training_doc"
#    testingFolder = "Testing_doc"
    path = os.getcwd()
    
    #Param setting
#    feature = 'sift'
    clustering_method = 'Crisp'
#    classification_method = 'Fuzzy'
    ...
    if(feature == 'SIFT' or feature == 'SURF'):
        ...
        for ind in range(0,noOfClass):
            ...
            for indFile in range(0,len(dirsTF)):
                image = IA_readSource(pathTrainingClass + '\\' + dirsTF[indFile], verbose)
                imageKeyPoint = FE_keypointDetection(image, feature, verbose)
        ...
        cluster = FE_Clustering(imageKeyPoint_all, clustering_method, 5)
        
        
        #Keypoint quantisation
        counter4 = 0;
        for indData in range(0,len(data[1])):
            quantisation = FE_Quantisation(data[2][indData], clustering_method, cluster[0])
        ...
        for ind in range(0,noOfClass):
            ...
            for indFile in range(0,len(dirsTF)):
                image = IA_readSource(pathTrainingClass + '\\' + dirsTF[indFile], verbose)
                imageFea = FE_LBPDetection(image, verbose)   
    ...
    for ind in range(0,noOfClass):
        ...
        for indFile in range(0,len(dirsTest)):
            image = IA_readSource(pathTestingClass + '\\' + dirsTest[indFile], verbose)  
            
            if(feature == 'sift' or feature == 'surf'):
                imageKeyPoint = FE_keypointDetection(image, feature, verbose)
                desctest = FE_Quantisation(imageKeyPoint[1], clustering_method, cluster[0])
            elif(feature == 'LBP'):
                desctest = FE_LBPDetection(image, verbose) 
